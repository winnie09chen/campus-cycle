...
| `index.html` | 学生（买家/卖家/援助池/我的交易） | 买家发布需求并匹配（筛选/排序/分页）；卖家上传照片识别+文案（自动压缩+进度条）、提交审核；困难生定向援助；模拟钱包与托管交易、订单评价、站内消息中心 |
| `reviewer.html` | 审核员 | 认证学生、审核物品（含批量通过/驳回）、处理举报、评价管理、审核日志、添加审核员、交易总览（AI 分析） |
...
- 顶栏「消息」入口（未读角标），面板内查看/一键已读
- 下单、审核通过/驳回、援助锁定/完成、交易完成等关键节点自动产生通知（`notify-module.js`）
- 通知随共享数据层（`data.notifications`）跨端同步

## 心愿单与需求风向标

- **买家**：检索结果卡可「☆ 加入心愿单」（每用户每类别+科目一条，预算取当前需求）；「个人中心 → 我的心愿单」可查看/删除
- **卖家**：「我要卖」tab 的「需求风向标」= 心愿单 + 未匹配需求按「类别+科目」聚合计数（预算中位数），**不展示任何用户信息**；「按这个需求发布」一键带入表单
- **通知**：物品**审核通过时**（单条/批量/审核员修正），扫描心愿单——类别相同 + 科目双向包含 + 非卖家本人 → 买家消息中心收到「心愿单上新」（未读角标随 3s 同步亮起）
- 数据存 `data.wishlist`，随共享数据层跨端同步；与本地「收藏」（❤️，仅自己可见）相互独立

## 评价与信誉

- 订单完成后买家可弹窗打星评价（1-5 星 + 评论），评价落库 `data.reviews`
- 卖家物品卡显示累计信誉徽章（好评率/星数）
- 审核员端「评价管理」tab 查看全部评价

## 公共模块（前端）

- `common.css`：设计 token 唯一来源（温暖手账风）+ 加载/空态/骨架屏组件
- `utils.js`：escapeHTML / 防抖节流 / showToast / confirmDialog 等通用工具
- `data-module.js`：数据层统一（loadData/saveData/远程同步/图片上传，远程请求自动带鉴权头）
- `notify-module.js`：站内通知 CRUD 与消息中心 UI
- `image-compressor.js`：Canvas 图片压缩、缩略图、格式校验与进度反馈
